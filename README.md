...
It is one HTML file. No build, no dependencies. A server is optional: the relay
in `interop/` is a hundred lines, does three jobs, and holds none of your data.

```
byline.html              the whole application (~500KB, one file)
...
interop/relay-test.mjs   drives the relay's guards from outside: token, allowlist, rate limit
...
interop/deliver.py       refreshes the Downloads copies and prints the close-out checklist
...
.claude/skills/          six skills so the next session does not re-derive any of this
...
never by hand: a 470KB inline script cannot be read into an editor's context
...
177 of them, in-browser, no runner. Each one is either a bug that shipped once
...
The named coding harnesses — Claude Code, Codex, goose — are switchable labels.
A page cannot start a local process, and the interface says so on every one of
them. Webhook deliveries are simulated unless a relay is set, in which case they
...
Still not done, because it cannot be done from here: everything under
"anyone but me" in the list, mdoc (a government-document format, doable but
pointless for a newsroom), and KYA-OS's wire protocol (read, scoped, not yet
built).

`THREAT-MODEL.md` is the long version, and it leads with the gaps.
